...
class SignalViewer(QMainWindow):
    ...
    def open_dialog_box(self, graphic_channel,button, timer, repeat_checkbox):
        ...
        if path:
            if button == self.BrowseButton:
                ...
                self.signal_widget1.signals.append(self.signal_widget1.load_signal(path, 0))

                if not graphic_channel.layout():
            ...
            if button == self.BrowseButton2:
                ...
                self.signal_widget2.signals.append(self.signal_widget2.load_signal(path, 0))
    ...
    def move_signal_down_handler(self):
        ...
        if layout1 and layout1.count() > 0:
            ...
            self.move_signal_between_widgets(self.signal_widget1, self.signal_widget2)
    ...
    def move_signal_up_handler(self):
        ...
        if layout2 and layout2.count() > 0:
            ...
            self.move_signal_between_widgets(self.signal_widget2, self.signal_widget1)
    ...
    def move_signal_between_widgets(self, source_signal_widget, dest_signal_widget):
        if source_signal_widget and dest_signal_widget:
            source_signal_widget.get_plot_widget().setVisible(False)
            source_channel = self.Channel1Graphic if source_signal_widget.pos_flag == 1 else self.Channel2Graphic
            source_channel.setText("Show")

            dest_channel = self.Channel1Graphic if dest_signal_widget.pos_flag == 1 else self.Channel2Graphic
            dest_channel.setLayout(source_channel.layout())
            dest_channel.setSceneRect(0, 0, dest_channel.width(), dest_channel.height())
            dest_channel.setScene(QGraphicsScene())
            dest_channel.layout().addWidget(dest_signal_widget.get_plot_widget())

            dest_signal_widget.pos_flag, source_signal_widget.pos_flag = source_signal_widget.pos_flag, dest_signal_widget.pos_flag
            dest_signal_widget.statistics_field1, source_signal_widget.statistics_field1 = source_signal_widget.statistics_field1, dest_signal_widget.statistics_field1
            source_signal_widget.statistics_field2.clear()
...
class Signal:
    ...
    def load_signal(self, path, selected_channel_index):
        ...
        return (self.ecg_signal, self.time)
    ...
    def update(self, timer):
        if self.deleted or not self.playing:
            return

        x_min_list, x_max_list = [], []
        curves = []  # List to store the curves for each signal
        for i, (ecg_signal, time) in enumerate(self.signals):
            if self.ptr >= len(time):
            ...
            signal_slice = ecg_signal[:self.ptr + 1]
            ...
            if not self.curve_deleted and self.curve is not None:
                if i < len(curves):
                    curve = curves[i]
                    curve.setData(time[:self.ptr + 1], signal_slice)
                else:
                    curve = self.plot_widget.plot(pen=(i, len(self.signals) * 1.3))
                    curve.setData(time[:self.ptr + 1], signal_slice)
                    curves.append(curve)  # Append the newly created curve
        ...
        x_min_final = min(x_min_list) if x_min_list else 0
        x_max_final = max(x_max_list) if x_max_list else self.fixed_x_range
        y_min_final = min(np.min(signal) for signal, _ in self.signals) if self.signals else 0
        y_max_final = max(np.max(signal) for signal, _ in self.signals) if self.signals else 1
        y_padding = 0.5
        self.plot_item.setXRange(x_min_final, x_min_final + self.fixed_x_range, padding=0)
        self.plot_widget.setLimits(xMin=0, xMax=max(self.fixed_x_range, x_max_final + 0.5), yMin=y_min_final - y_padding, yMax=y_max_final + y_padding)
